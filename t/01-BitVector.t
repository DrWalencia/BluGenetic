...
"setGen: Introduce something different than a bit e.g = -4 and check if it returns false";
}
...
# setGen: Introduce an element in a wrong position (over or above genotypeLength)
{
    ...
    "setGen: introduce an element in a negative position";
    dies_ok { ( $genotype->setGen( 7, 0 ) ) }
    "setGen: introduce an element in a position bigger than genotypeLength";
}
...
# getGen: Retrieve an element from a wrong position (over or above genotypeLength)
{
    ...
    "getGen: retrieve an element from a negative position";
    dies_ok { ( $genotype->getGen(7) ) }
    "getGen: retrieve an element from a position bigger than genotypeLength";
}
...
# changeGen: create a BitVector element, set its genes manually and flip a given bit
{
    ...
    ok(
        $genotype->getGen(1) == 1,
"changeGen: create a BitVector element, set its genes manually and flip a given bit (0-1)"
    );

    $genotype->changeGen(1);
    ok(
        $genotype->getGen(1) == 0,
"changeGen: create a BitVector element, set its genes manually and flip a given bit (1-0)"
    );
}
...
# changeGen: try to flip bit in wrong position
{
    ...
    "changeGen: try to flip bit in a negative position";
    dies_ok { ( $genotype->changeGen(5) ) }
    "changeGen: try to flip bit in a position bigger than genotypeLength";
}
