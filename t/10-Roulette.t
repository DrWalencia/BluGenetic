...
# performSelection: over an empty population. Dies painfully
{
    ...
    dies_ok { $selection->performSelection(@population) }
    "performSelection: over an empty population. Dies painfully";
}
...
# filled only with copies of the first element
{
    ...
    ok( @selectedPop,
        "Population result of the selection strategy is not empty" );
    ...
    for ( my $i = 0 ; $i < @selectedPop ; $i++ ) {
        ...
        ok( $ind1->getScore() == $ind2->getScore(),
            "Check that element 0 and element $i have the same score" );
        ...
        for ( my $j = 0 ; $j < $genotype1->getLength() ; $j++ ) {
            ok(
                $genotype1->getGen($j) == $genotype2->getGen($j),
                "Gen $j of both elements have the same value"
            );
        }
    }
...
# score. The accumulated normalized scores must be 0.5 and 1
{
    ...
    ok(
        $sortedNormalizedPop[-1]->getScore() == 1,
"Score as expected for last individual as a product of selection strategy"
    );
    ok(
        $sortedNormalizedPop[0]->getScore() == 0.5,
"Score as expected for first individual as a product of selection strategy"
    );

    ok(
        $sortedNormalizedPop[0]->getGenotype()->getGen(0) !=
          $sortedNormalizedPop[-1]->getGenotype()->getGen(0),
        "The individuals are different in genotype"
    );
}
...
"Score as expected for last individual as a product of selection strategy"
    ...
    ok(
        $sortedNormalizedPop[0]->getScore() == 0.75,
"Score as expected for first individual as a product of selection strategy"
    );
