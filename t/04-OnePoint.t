...
# indOne.getGenotype.GetLength()
{
    ...
    for ( my $j = 0 ; $j < $v[0]->getGenotype()->getLength() - 1 ; $j++ ) {
        ok( $v[0]->getGenotype->getGen($j) == 0,
            "crossIndividuals: first child has 0 in position $j" );
    }

    # And a one in the last position
    ok( $v[0]->getGenotype->getGen(5) == 1,
        "crossIndividuals: first child has 1 in the last pos" );

    # The second child will have ones in the first five positions
    for ( my $j = 0 ; $j < $v[1]->getGenotype()->getLength() - 1 ; $j++ ) {
        ok( $v[1]->getGenotype->getGen($j) == 1,
            "crossIndividuals: second child have 1 in pos $j" );
    }

    # And a zero in the last position
    ok( $v[1]->getGenotype->getGen(5) == 0,
        "crossIndividuals: second child has 0 in last pos" );
}
