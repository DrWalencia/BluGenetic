...
# setGen: value not in set of possible values. Dies
{
	...
	dies_ok{$genotype->setGen(2,"thinnest")} "setGen: fails if value below limits";
}
...
# setGen: make sure a blue is inserted in gen 0 by inserting it
{
	...
	ok ($genotype[0] eq "blue", "setGen: make sure a blue is inserted in gen 0 by inserting it");
}
...
# encapsulation are the same
{
	...
	ok ($genotype[0] eq $element, "getGen: check that the gen returned and the one got by breaking encapsulation are the same");
}
