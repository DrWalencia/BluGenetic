...
use fields 
    'genotype', # Array of genes belonging to an individual, e.g:[3,-2,4]
...
sub setGen {
    ...
    $log->logconfess( "Value out of ranges: $value (Ranges: [",
        $myRange[0], ",", $myRange[1], "])" )
...
sub changeGen {
    ...
    my $value =
      int( rand( abs( $myRange[1] - $myRange[0] + 1 ) ) ) + $myRange[0];
